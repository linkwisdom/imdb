...
//     };
     ...
     * 包装请求对象为Deferred对象
     *
     * @param {IDBRequest} idb请求对象
     * @param {Object=} 请求上下文
     * @param {Function=} 请求回调处理
     */
...
//        context.version = dbConf.version;
    ...
    exports.update = function (selector, context, option) {
        ...
        if (typeof selector == 'object' && !Array.isArray(selector)) {
            ...
            throw new Error('the parameter update{@selector} must be Map-Object');
        }
    };
    ...
    exports.insert = function (sets, context) {
        ...
        transaction.oncomplete = function (e) { 
            if (restList.length > 0) {
                ...
                console.log('loopCount %s', context.loopCount);
        ...
        if (sets.length > SPLICE_SIZE) {
            var segments = sets.slice(0, SPLICE_SIZE);
            // 剩余数据到下次执行
...
/// adding test
    ...
    exports.find = function (selector, context, callback) {
        ...
        if (!filter && store.indexNames.contains[store.keyPath]) {
            // 如果没有指定查询条件，默认按主键查询
            var index = store.index(store.keyPath);
